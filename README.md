...
1. 遍历所有日志条目以找到最大页号，将页面缓存截断到最大页号
2. redo
3. undo

🎈一个小知识

关于为什么需要进行页号截断，原作者没写，百思不得其解，后来查阅资料发现原因：

页面缓存中可能包含由于崩溃或故障引入的“未使用”页面。特别是在发生故障时，缓存中的一些页面可能已经分配了页号但没有实际使用，因此日志中不一定有这些页面的记录。恢复时截断到最大页号可以清理这些未使用页面，避免恢复时加载无效数据。

在截断时有一步操作是将maxPgno设为1，原因是第一页为特殊页不会出现在日志中
